...
INTERNATIONAL_COMMANDS: list[HsmCommand] = [
    ...
    HsmCommand("Thales/Futurex", "International", "KQ",
               ...
               "+ Pre-formatted PAN/PAN_Seq 8 B (16 BCD digits, EMV Option A per EMV Book 2 Annex A1.4.1: "
               "the RIGHTMOST 16 digits of (PAN || 2-digit PSN), LEFT-padded with ZEROS — "
               "NOT left-justified and NOT 0xF-padded). Decode: PSN = last 2 digits, PAN = first 14 digits. "
               "Example: PAN 123456789012, PSN 01 -> 00 12 34 56 78 90 12 01 (\"0012345678901201\"), "
               "NOT \"12345678901201FF\". Mis-reading this as left-justified/0xF-padded (split at 12 digits) "
               "corrupts both PAN and PSN for any 16-digit PAN and breaks ICC-master-key derivation -> ARQC "
               "verification fails. APC verify_auth_request_cryptogram / SessionKeyEmvCommon re-applies Option A "
               "internally, so pass PAN = first-14-digits and PSN = last-2-digits. "
               "(Same field is shared by KW/KU/K2/K0/KS.) "
    ...
    HsmCommand("Thales/Futurex", "International", "KW",
               ...
               "Wire: same binary-encoded structure as KQ (including the EMV Option A pre-formatted "
               "PAN/PAN_Seq field — rightmost-16 of PAN||PSN, left zero-padded) plus Derivation Method 1A "
