...
- Cite by identifier (e.g. "PUGD0537-004 p.488", "EMV Book 2 v4.3 §8.2.1"), matching a Sources-ledger row. When a claim is verified against a held document, say so; when the source is **not held** (the paywalled ISO/X9 texts, or the CIAM-gated Futurex command reference), say that instead — never imply a version we don't have.
