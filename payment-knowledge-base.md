...
attributes:
  ...
  key_differences_from_kq:
    ...
    differences:
      ...
      session_key_derivation: "KQ: SessionKeyDerivation::EmvCommon (Visa/MC). JS: SessionKeyDerivation::UnionPay (CUP/PBOC) — PAN + PSN + ATC; APC added the native UNION_PAY mode 2026-07-15 (boto3 >= 1.43.49). Previously mapped to Emv2000 as a stand-in because CUP/PBOC is EMV-2000 based; both take the same three inputs, so the old mapping's correctness is UNVERIFIED — confirm UNION_PAY against live APC or a CUP test vector."
