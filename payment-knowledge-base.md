...
domain:
  - emv
references:
  - "EMV Book 3 Application Specification v4.3 Annex A — held PDF (EMV Book 3 v4.3; see AGENTS.md → Reference Source Documents), these tags verified present 2026-07-06; Sources ledger 2026-07-06. (v4.4 is the current edition but is not held; these tag definitions are edition-stable.)"
  - "Open payment tooling analysis, Sources ledger 2026-05-14 (definitional ingest)"
status: active
```
...
to v4.3 either. Tags that also appear in v4.3 can be cross-checked against the held EMV Book 3 v4.3. This
...
Source: EMV Integrated Circuit Card Specifications for Payment Systems, Book 2 — Security and Key Management. v4.3 (November 2011) is HELD (see AGENTS.md → Reference Source Documents) and re-verifiable; the v4.4 (November 2023) corrections below (Bulletin 208 RSA modulus, ECC additions) are from a one-time read on 2026-05-22 — v4.4 is NOT held and those specific corrections cannot be re-verified against a held file. Entries use v4.3 structure with v4.4 corrections annotated as such.
...
relationships:
  ...
  - type: related_to
    target_id: operation.emv-arqc-generation
references:
  - "EMV Book 2 Annex B (Approved Algorithms) + Bulletin 208. v4.4-specific content (ECC XDA/ODE suites, Bulletin 208 RSA-modulus correction) from a one-time v4.4 read on 2026-05-22 (Sources ledger); v4.4 is NOT held on disk and this is not re-verifiable against a held file. The RSA/hash/TDES fundamentals also appear in the held EMV Book 2 v4.3 Annex B (see AGENTS.md → Reference Source Documents)."
status: active
```
...
attributes:
  ...
  key_differences_from_kq:
    ...
    differences:
      ...
      session_key_derivation: "KQ: SessionKeyDerivation::EmvCommon (Visa/MC). JS: SessionKeyDerivation::UnionPay (CUP/PBOC) — PAN + PSN + ATC; APC added the native UNION_PAY mode 2026-07-15 (boto3 >= 1.43.49). Previously mapped to Emv2000 as a stand-in because CUP/PBOC is EMV-2000 based. VERIFIED live (us-east-1, apc-hsm-proxy arqc_verify_js_differential): mint under EMV2000 + verify under UNION_PAY accepts 32/32 across randomized PAN/PSN/ATC/txn length, one-bit-corrupted ARQC still rejected — the two derivations are functionally identical over PAN + PSN + ATC. Switch for scheme faithfulness, not behaviour."
...
| 2026-08-22 | AWS Payment Cryptography release sweep (2026-06 → 2026-08) against the botocore service models — the authoritative signal for API-shape changes. DATA PLANE 2026-07-15 (botocore/boto3 1.43.49): UnionPay session key derivation added — new SessionKeyUnionPay member (PrimaryAccountNumber + PanSequenceNumber + ApplicationTransactionCounter, all required) on the SessionKeyDerivation union used by GenerateAuthRequestCryptogram and VerifyAuthRequestCryptogram, plus UNION_PAY on the flat SessionKeyDerivationMode enum used by GenerateMac/VerifyMac EmvMac attributes. This SUPERSEDES the prior claim that UnionPay/CUP had no APC SessionKeyDerivation equivalent (payShield KW Scheme ID 'C', JS command) — that claim was correct when validated 2026-06-16 and is now obsolete. CONTROL PLANE 2026-06-05: documentation-only — HMAC key check values are computed with the hash selected at key creation over a zero-length message, taking the leftmost 3 bytes (previously only TDES and AES KCV were documented). NOT YET COVERED by this server: Multi-Party Approval ops (AssociateMpaTeam/DisassociateMpaTeam/GetMpaTeamAssociation, 2026-04-30) incl. ImportKey.RequesterComment and Key.MpaStatus; the five key-replication-region ops (2025-09-10); GetCertificateSigningRequest (2025-09-12); GetPublicKeyCertificate | AWS service models via botocore git history (boto/botocore, botocore/data/payment-cryptography{,-data}) + AWS What's New + awsapichanges.info per-service pages; UnionPay-vs-EMV2000 equivalence verified live in us-east-1 by apc-hsm-proxy test arqc_verify_js_differential | read 2026-08-22, extended 2026-08-23 | emv, card_validation, hsm, key_management |
